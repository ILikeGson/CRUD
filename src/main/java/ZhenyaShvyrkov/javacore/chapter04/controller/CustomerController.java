...
public class CustomerController {
    ...
    private JavaIOSpecialtyRepositoryImpl specialtyRepository = new JavaIOSpecialtyRepositoryImpl();
    ...
    public Customer create(String[] data){
        Account account = accountRepository.save(new Account(data[1],data[2],Integer.parseInt(data[3]),ascertainStatus(data[4])));
        Set<Specialty> set = new HashSet<Specialty>();
        for(int i = 5; i < data.length; i++){
            Specialty specialty = specialtyRepository.save(new Specialty(data[i]));
            set.add(specialty);
        }
    ...
    public Customer update(String[] data){
        Account account = accountRepository.update(Long.valueOf(data[1]), new Account(data[2], data[3], Integer.parseInt(data[4]),ascertainStatus(data[5])));
        Set<Specialty> set = new HashSet<Specialty>();
        Customer customer = customerRepository.getByID(Long.valueOf(data[1]));
        customer.getSpecialties().stream().forEach(specialtyRepository::delete);
        for(int i = 6; i < data.length;i++){
            set.add(specialtyRepository.save(new Specialty(data[i])));
        }
    ...
    public void deleteById(Long id){
        ...
        for(Specialty element : customer.getSpecialties()){
            specialtyRepository.delete(element);
        }
